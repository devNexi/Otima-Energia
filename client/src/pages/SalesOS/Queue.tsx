...
const STAT_CHIPS = [
  { label: "Chamadas Hoje", value: allStats.calls, icon: <Phone size={13} />, color: "#9e3ffd" },
  { label: "DMs Alcançados", value: allStats.dms, icon: <Users size={13} />, color: "#22c55e" },
  { label: "Contas Solicitadas", value: allStats.requested, icon: <FileText size={13} />, color: "#f59e0b" },
  { label: "Contas Recebidas", value: allStats.received, icon: <FileCheck size={13} />, color: "#3b82f6" },
  { label: "Callbacks Pendentes", value: allStats.callbacks, icon: <Clock size={13} />, color: "#ef4444" },
];
...
function PreviewPanel({ lead }: { lead: Lead | null }) {
  ...
  if (!lead) return (
    <div className="flex flex-col items-center justify-center h-full gap-3" style={{ color: "rgba(255,255,255,0.2)" }}>
      <Building2 size={32} />
      <span className="text-sm">Selecione um lead para visualizar</span>
    </div>
  );
  ...
  return (
    <motion.div key={lead.id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="p-5 space-y-4">
      ...
      <div className="rounded-lg p-3" style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.07)" }}>
        <div className="text-[10px] uppercase tracking-wide mb-1.5" style={{ color: "rgba(255,255,255,0.35)" }}>Decisor</div>
        {lead.dm_name ? (
          <div>
          ...
          <em className="text-xs" style={{ color: "rgba(255,255,255,0.3)" }}>Decisor não identificado</em>
        )}
      </div>

      {/* Priority label */}
      <div>
        <div className="text-[10px] uppercase tracking-wide mb-1" style={{ color: "rgba(255,255,255,0.35)" }}>Prioridade</div>
        <span className={`text-xs font-semibold uppercase ${pConf.textColor}`}>{pConf.label}</span>
      </div>

      {/* Status */}
      <div>
        <div className="text-[10px] uppercase tracking-wide mb-1" style={{ color: "rgba(255,255,255,0.35)" }}>Status</div>
        <div className="text-xs" style={{ color: "rgba(255,255,255,0.65)" }}>{lead.lead_status}</div>
      </div>

      {/* Last outcome */}
      <div>
        <div className="text-[10px] uppercase tracking-wide mb-1" style={{ color: "rgba(255,255,255,0.35)" }}>Último Resultado</div>
      ...
      {/* Next action */}
      <div>
        <div className="text-[10px] uppercase tracking-wide mb-1" style={{ color: "rgba(255,255,255,0.35)" }}>Próxima Ação</div>
      ...
      <div className="flex items-center justify-between">
        <div className="text-[10px] uppercase tracking-wide" style={{ color: "rgba(255,255,255,0.35)" }}>Score de Prioridade</div>
      ...
      <div className="flex flex-col h-screen overflow-hidden">
        ...
        <div className="shrink-0 px-6 py-4 border-b" style={{ borderColor: "rgba(255,255,255,0.07)", background: "rgba(0,0,0,0.2)" }}>
          <div className="flex items-center justify-between gap-4 mb-4">
            <div>
              <h1 className="text-lg font-bold" style={{ color: "#fff" }}>Minha Fila</h1>
              <div className="text-xs" style={{ color: "rgba(255,255,255,0.4)" }}>{filtered.length} leads · ordenados por prioridade</div>
            </div>
          ...
          <div className="flex gap-3 flex-wrap">
            {STAT_CHIPS.map(chip => (
              <div key={chip.label} className="flex items-center gap-2 px-3 py-1.5 rounded-lg" style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.07)" }}>
          ...
          {highPriorityClear && (
            <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }}
              ...
              <span className="text-sm" style={{ color: "#86efac" }}>
                Fila de prioridade limpa — você pode trabalhar novos leads da Julia agora. Ótimo trabalho.
              </span>
            </motion.div>
          )}
        </AnimatePresence>
          ...
          <div className="overflow-y-auto p-4 space-y-3" style={{ width: "65%", borderRight: "1px solid rgba(255,255,255,0.06)" }}>
            {filtered.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-48 gap-3" style={{ color: "rgba(255,255,255,0.3)" }}>
                ...
                <div className="text-sm">Nenhum lead atribuído. Fale com o Renan.</div>
              </div>
            ) : (
              filtered.map((lead, i) => (
                <QueueItem
