...
const PROJECTS = [
  {
    name: "Sales OS",
    desc: "Operational platform for sales team management",
    delivery: "This week",
    status: "In progress",
    statusColor: "#f59e0b",
    progress: 72,
    ...
    blockers: ["Call Assist awaiting UX decision"],
  },
  {
    name: "MT (Mercado Transparente)",
    desc: "Energy price comparison platform for businesses",
    delivery: "2 weeks",
    status: "In progress",
    statusColor: "#9e3ffd",
    progress: 45,
  ...
  {
    name: "SOP Agents",
    desc: "AI agents for executing operational SOPs",
    delivery: "3 weeks",
    status: "In progress",
    statusColor: "#16a34a",
    progress: 30,
    ...
    deliverables: [
      ...
      { label: "Quality testing (QA pass)", done: false },
      { label: "Production deploy",        done: false },
    ],
    blockers: ["SOP 7 and SOP 11 with logic errors"],
  },
];
      ...
      <div className="h-screen overflow-y-auto" style={{ background: "#F8F9FC" }}>
        <div className="px-6 py-5 border-b" style={{ background: "#FFFFFF", borderColor: "#E8EAED" }}>
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-xl font-bold" style={{ color: "#16163f" }}>Product</h1>
        ...
        <div className="p-6 space-y-5">
          ...
          <div className="rounded-2xl p-5" style={{ background: "#FFFFFF", border: "1px solid #E8EAED" }}>
            <div className="flex items-center gap-2 mb-4">
              ...
              <div className="font-bold text-sm" style={{ color: "#16163f" }}>Dev Team Cost</div>
            </div>
            <div className="grid grid-cols-4 gap-4">
              {[
                { label: "Current monthly cost",    value: `R$ ${MONTHLY_COST.toLocaleString("en-GB")}`,           color: "#dc2626" },
                { label: "After reduction (~2 mo)", value: `R$ ${REDUCED_COST.toLocaleString("en-GB")}`,           color: "#16a34a" },
                { label: "Monthly savings",         value: `R$ ${(MONTHLY_COST - REDUCED_COST).toLocaleString("en-GB")}`, color: "#f59e0b" },
                { label: "Total invested to date",  value: `R$ ${totalSpent.toLocaleString("en-GB")}`,             color: "#374151" },
              ].map(k => (
              ...
              <div key={p.name} className="rounded-2xl p-5" style={{ background: "#FFFFFF", border: "1px solid #E8EAED" }}>
                ...
                {p.blockers.length > 0 && (
                  <div className="mt-4 rounded-lg p-2.5" style={{ background: "#fef2f2", border: "1px solid #fecaca" }}>
                    ...
                    {p.blockers.map(b => <div key={b} className="text-[11px]" style={{ color: "#991b1b" }}>{b}</div>)}
                  </div>
                )}
                <div className="mt-3 pt-3 flex items-center justify-between" style={{ borderTop: "1px solid #F3F4F6" }}>
                  <div className="text-[10px]" style={{ color: "#9CA3AF" }}>Cost to date</div>
                  <div className="text-xs font-bold" style={{ color: "#374151" }}>R$ {p.costToDate.toLocaleString("en-GB")}</div>
                </div>
              </div>
            ))}
          </div>
          ...
          {/* Timeline */}
          <div className="rounded-2xl p-5" style={{ background: "#FFFFFF", border: "1px solid #E8EAED" }}>
            <div className="font-bold text-sm mb-4" style={{ color: "#16163f" }}>Delivery Timeline</div>
            <div className="space-y-3">
              {PROJECTS.map(p => {
                const weeks = ["W1 (this week)", "W2", "W3", "W4", "W5", "W6"];
                  ...
                  <div key={p.name} className="flex items-center gap-4">
                    ...
                    <div className="flex-1 relative h-8 flex items-center">
                      {weeks.map((w, i) => (
                        <div key={w} className="flex-1 h-8 flex items-center justify-center relative"
                          ...
                          {i === 0 && <div className="absolute text-[9px]" style={{ color: "#9CA3AF", top: -14 }}>Today</div>}
                          {i <= endIdx && (
                            <div className="w-full h-5 rounded"
                              style={{ background: `${p.statusColor}${i === endIdx ? "40" : "20"}`,
                    ...
                    <div className="w-24 shrink-0">
                      <span className="text-[10px] px-2 py-0.5 rounded-full font-semibold"
                        style={{ background: `${p.statusColor}15`, color: p.statusColor }}>{p.delivery}</span>
                    </div>
                  </div>
                );
              })}
          ...
          {/* Cost efficiency */}
          <div className="rounded-2xl p-5" style={{ background: "#FFFFFF", border: "1px solid #E8EAED" }}>
            <div className="font-bold text-sm mb-4" style={{ color: "#16163f" }}>Cost Efficiency</div>
            <div className="grid grid-cols-3 gap-4">
              <div className="rounded-xl p-4" style={{ background: "#F8F9FC", border: "1px solid #E8EAED" }}>
                <div className="text-[10px] mb-1" style={{ color: "#9CA3AF" }}>Cost per milestone</div>
                <div className="text-xl font-bold" style={{ color: "#374151" }}>R$ {costPerMilestone.toLocaleString("en-GB")}</div>
                <div className="text-[10px] mt-1" style={{ color: "#9CA3AF" }}>6 milestones expected/month</div>
              </div>
              <div className="rounded-xl p-4" style={{ background: "#F8F9FC", border: "1px solid #E8EAED" }}>
                <div className="text-[10px] mb-1" style={{ color: "#9CA3AF" }}>Milestones delivered</div>
                <div className="text-xl font-bold" style={{ color: "#f59e0b" }}>4 of 12</div>
                <div className="text-[10px] mt-1" style={{ color: "#9CA3AF" }}>this cycle</div>
              </div>
              <div className="rounded-xl p-4" style={{ background: "#fef2f2", border: "1px solid #fecaca" }}>
                <div className="flex items-center gap-1 mb-1">
                  <TrendingDown size={11} style={{ color: "#dc2626" }} />
                  <div className="text-[10px]" style={{ color: "#dc2626" }}>Below planned speed</div>
                </div>
                <div className="text-xl font-bold" style={{ color: "#dc2626" }}>33%</div>
                <div className="text-[10px] mt-1" style={{ color: "#dc2626" }}>vs 100% planned</div>
              </div>
            </div>
          </div>
        </div>
