...
const MILESTONES = [
  { label: "Sales OS live with Zoho integration",          date: "This week",      done: true },
  { label: "Elayne and Thaina reach 2 bills/day",          date: "By June 30",     done: false },
  { label: "Oscar activates first agent",                  date: "By June 30",     done: false },
  { label: "Dev costs reduce 50%",                         date: "By August 2025", done: false },
  { label: "First partnership generates lead flow",        date: "By July 2025",   done: false },
];
  ...
  return (
    <ResponsiveContainer width="100%" height={height}>
      <LineChart data={data} margin={{ top: 8, right: 8, left: -10, bottom: 0 }}>
        ...
        <Tooltip
          formatter={(v: number, name: string) => [fmtBRL(v), name === "receita" ? "Revenue" : "Burn"]}
        ...
        {beDot && (
          <ReferenceDot x={beDot.month} y={beDot.receita} r={5} fill={color} stroke="#fff" strokeWidth={2}
            label={{ value: `BE Month ${breakEvenMonth}`, position: "top", fontSize: 9, fill: color }} />
        )}
      </LineChart>
    </ResponsiveContainer>
  );
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="rounded-2xl p-6" style={{ background: "#FFFFFF", border: `2px solid ${runwayColor}30`, boxShadow: "0 1px 4px rgba(0,0,0,0.06)" }}>
            ...
            <div className="flex justify-between text-[10px] mt-1" style={{ color: "#9CA3AF" }}>
              <span>Current MRR: {Math.round((MRR / TOTAL_BURN) * 100)}% of burn</span>
              <span>Break-even at 100%</span>
            </div>
          </div>
          ...
          <div className="rounded-2xl p-5" style={{ background: "#FFFFFF", border: "1px solid #E8EAED" }}>
            ...
            <div className="mt-3 text-[11px] text-center" style={{ color: "#9CA3AF" }}>
              Calculated automatically from your actual avg. client ticket ({fmtBRL(Math.round(avgTicket))}/month)
            </div>
          </div>
          ...
          <div className="rounded-2xl overflow-hidden" style={{ background: "#FFFFFF", border: "1px solid #E8EAED" }}>
            <div className="flex items-center gap-2 px-5 py-4 border-b" style={{ borderColor: "#F3F4F6" }}>
              ...
              <span className="font-semibold text-sm" style={{ color: "#16163f" }}>Plan Milestones</span>
            </div>
            {MILESTONES.map((m, i) => (
