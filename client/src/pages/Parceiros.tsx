  ...
  if (submitted) {
    return (
      <div className="text-center py-12 px-4 space-y-4">
        ...
        <p className="text-sm max-w-md mx-auto" style={{ color: "rgba(255,255,255,0.72)" }}>
  ...
  return (
    <div>
      <div className="flex gap-2 mb-8">
        {[1, 2, 3].map(n => (
          <div key={n} className="flex-1 h-1.5 rounded-full overflow-hidden" style={{ background: "rgba(255,255,255,0.08)" }}>
      ...
      {step === 1 && (
        <div className="space-y-4">
          ...
          {[
            ...
            <div key={field.key}>
              <label className="block text-xs font-medium mb-1.5" style={{ color: "rgba(255,255,255,0.4)" }}>{field.label}</label>
          ...
          <div>
            <label className="block text-xs font-medium mb-1.5" style={{ color: "rgba(255,255,255,0.4)" }}>Estado</label>
            <select value={f1.estado} onChange={e => setF1({ ...f1, estado: e.target.value })} className={inp} style={{ ...inpStyle, appearance: "none" }}>
              <option value="" style={{ background: "#13112a" }}>Selecione o estado</option>
              {ESTADOS.map(e => <option key={e} value={e} style={{ background: "#13112a" }}>{e}</option>)}
            </select>
          </div>
          <button onClick={() => {
      ...
      {step === 2 && (
        <div className="space-y-5">
          ...
          <div>
            <label className="block text-xs font-medium mb-2" style={{ color: "rgba(255,255,255,0.4)" }}>Você já vendeu B2B?</label>
            {radioOpts("b2b", [{ val: "sim", label: "Sim" }, { val: "nao", label: "Não" }], f2.vendeuB2b, v => setF2({ ...f2, vendeuB2b: v }))}
          </div>
          <div>
            <label className="block text-xs font-medium mb-2" style={{ color: "rgba(255,255,255,0.4)" }}>Qual sua experiência comercial?</label>
            {radioOpts("exp", [
          ...
          <div>
            <label className="block text-xs font-medium mb-2" style={{ color: "rgba(255,255,255,0.4)" }}>Horas por semana disponíveis</label>
            {radioOpts("horas", [{ val: "5-10", label: "5–10h" }, { val: "10-20", label: "10–20h" }, { val: "20+", label: "20h+" }], f2.horas, v => setF2({ ...f2, horas: v }))}
          </div>
          <div>
            <label className="block text-xs font-medium mb-2" style={{ color: "rgba(255,255,255,0.4)" }}>Você tem rede local com empresários?</label>
        ...
        <div className="space-y-5">
          ...
          {[
            ...
            <button key={item.key} type="button" onClick={() => setF3({ ...f3, [item.key]: !(f3 as any)[item.key] })}
              ...
              <span className="text-sm" style={{ color: (f3 as any)[item.key] ? "#ffffff" : "rgba(255,255,255,0.55)" }}>{item.label}</span>
            </button>
          ))}
          <div>
            <label className="block text-xs font-medium mb-1.5" style={{ color: "rgba(255,255,255,0.4)" }}>Por que você quer ser Parceiro Otima?</label>
...
export default function Parceiros() {
  ...
  return (
    <div className="parceiros-page" style={{ background: "#09081e", color: "#ffffff", fontFamily: "Inter, system-ui, sans-serif", overflowX: "hidden" }}>
      ...
      {/* ── Sticky header ── */}
      <header className="sticky top-0 z-50 px-5 py-3 flex items-center justify-between" style={{ background: "rgba(13,12,43,0.97)", backdropFilter: "blur(16px)", borderBottom: "1px solid rgba(158,63,253,0.15)" }}>
        <Link href="/">
      ...
      {/* ── 2. PROBLEM ── */}
      <section className="px-5 py-20 border-t border-[rgba(255,255,255,0.04)]" style={{ background: "#13112a" }}>
        <div className="max-w-5xl mx-auto">
          <Reveal>
            <SectionTag text="O problema" />
            <h2 className="text-3xl sm:text-4xl font-bold mb-6 text-white">Vender energia sem dados é <span style={{ color: "#9e3ffd" }}>começar no escuro.</span></h2>
            <p className="text-base mb-12 max-w-2xl" style={{ color: "rgba(255,255,255,0.72)" }}>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr style={{ background: "rgba(255,255,255,0.05)" }}>
                    <th className="text-left px-5 py-3.5 font-semibold" style={{ color: "rgba(255,80,80,0.8)", width: "50%" }}>Antes da Otima</th>
                ...
                <tbody>
                  {[
                    ...
                    <tr key={i} style={{ borderTop: "1px solid rgba(255,255,255,0.05)" }}>
                      <td className="px-5 py-3.5 flex items-start gap-2" style={{ background: "rgba(255,80,80,0.05)" }}>
                        <XCircle className="w-4 h-4 shrink-0 mt-0.5" style={{ color: "rgba(255,120,120,0.8)" }} />
                        <span style={{ color: "rgba(255,120,120,0.8)" }}>{before}</span>
                      </td>
                      <td className="px-5 py-3.5" style={{ background: "rgba(34,197,94,0.05)" }}>
                        <span className="flex items-start gap-2">
                          <CheckCircle2 className="w-4 h-4 shrink-0 mt-0.5" style={{ color: "#4ade80" }} />
      ...
      {/* ── 3. SOLUTION ── */}
      <section className="relative px-5 py-20 overflow-hidden border-t border-[rgba(255,255,255,0.04)]" style={{ background: "#09081e" }}>
        <div className="max-w-5xl mx-auto">
          <Reveal>
            <SectionTag text="O que você recebe" />
            <h2 className="text-3xl sm:text-4xl font-bold mb-4 text-white">A Otima te entrega uma <span style={{ color: "#9e3ffd" }}>base comercial para executar.</span></h2>
            <p className="text-base mb-12 max-w-2xl" style={{ color: "rgba(255,255,255,0.72)" }}>
            ...
            {[
              ...
              <Reveal key={i} delay={i * 60}>
                <div className="rounded-2xl p-5 h-full transition-all duration-200 hover:scale-[1.02]" style={{ background: cardBg, border: cardBorder }}>
                  <div className="w-10 h-10 rounded-xl flex items-center justify-center mb-4" style={{ background: "rgba(255,255,255,0.08)", border: `1px solid ${card.color}40` }}>
                    <card.icon className="w-5 h-5" style={{ color: card.color }} />
                  </div>
                  <h3 className="font-semibold mb-1.5 text-white">{card.title}</h3>
                  <p className="text-sm leading-relaxed" style={{ color: "rgba(255,255,255,0.72)" }}>{card.desc}</p>
                </div>
              </Reveal>
            ))}
          </div>
      ...
      {/* ── 4. HOW IT WORKS ── */}
      <section id="como-funciona" className="px-5 py-20 border-t border-[rgba(255,255,255,0.04)]" style={{ background: "#13112a" }}>
        <div className="max-w-4xl mx-auto">
          <Reveal>
            <SectionTag text="Como funciona" />
            <h2 className="text-3xl sm:text-4xl font-bold mb-12 text-white">Como funciona o <span style={{ color: "#9e3ffd" }}>Programa de Parceiros Otima</span></h2>
          </Reveal>
              ...
              <Reveal key={step.num} delay={i * 50}>
                <div className="flex gap-5 pb-8 relative">
                  ...
                  <div className="pb-2">
                    ...
                    <p className="text-sm leading-relaxed" style={{ color: "rgba(255,255,255,0.72)" }}>{step.desc}</p>
                  </div>
                </div>
              </Reveal>
            ))}
      ...
      <section className="px-5 py-20 border-t border-[rgba(255,255,255,0.04)]" style={{ background: "#13112a" }}>
        <div className="max-w-5xl mx-auto">
          <Reveal>
            ...
            <h2 className="text-3xl sm:text-4xl font-bold mb-4 text-white">Isto não é pirâmide. <span style={{ color: "#9e3ffd" }}>E nunca será.</span></h2>
            <p className="text-base mb-4 max-w-2xl" style={{ color: "rgba(255,255,255,0.72)" }}>
            ...
            <p className="text-base mb-10 max-w-2xl" style={{ color: "rgba(255,255,255,0.72)" }}>
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {[
                    ...
                    <tr key={i} style={{ borderTop: "1px solid rgba(255,255,255,0.05)" }}>
                      <td className="px-5 py-3.5" style={{ background: "rgba(255,80,80,0.05)" }}><span style={{ color: "rgba(255,120,120,0.8)" }}>{bad}</span></td>
                      <td className="px-5 py-3.5" style={{ background: "rgba(34,197,94,0.05)" }}><span style={{ color: "#4ade80" }}>{good}</span></td>
                    </tr>
                  ))}
                </tbody>
              </table>
      ...
      {/* ── 7. ENERGY BILL ── */}
      <section className="px-5 py-20 border-t border-[rgba(255,255,255,0.04)]" style={{ background: "#09081e" }}>
        <div className="max-w-4xl mx-auto text-center">
          <Reveal>
            <SectionTag text="O ponto de validação" />
            <h2 className="text-3xl sm:text-4xl font-bold mb-6 text-white">A conta de energia é <span style={{ color: "#9e3ffd" }}>o que importa.</span></h2>
            <p className="text-base mb-6 max-w-2xl mx-auto" style={{ color: "rgba(255,255,255,0.72)" }}>
          ...
          <Reveal delay={100}>
            <div className="flex flex-col sm:flex-row items-center justify-center gap-6 my-10">
              <div className="rounded-2xl px-8 py-6" style={{ background: "rgba(255,80,80,0.08)", border: "1px solid rgba(255,80,80,0.2)" }}>
                ...
                <p className="text-sm mt-1" style={{ color: "rgba(255,255,255,0.55)" }}>é contacto.</p>
              </div>
              ...
              <div className="rounded-2xl px-8 py-6" style={{ background: "rgba(158,63,253,0.1)", border: "1px solid rgba(158,63,253,0.3)" }}>
                ...
                <p className="text-sm mt-1" style={{ color: "rgba(255,255,255,0.55)" }}>vira oportunidade.</p>
              </div>
            </div>
          </Reveal>
        </div>
      ...
      <section className="px-5 py-20 border-t border-[rgba(255,255,255,0.04)]" style={{ background: "#13112a" }}>
        <div className="max-w-5xl mx-auto">
          <Reveal>
            ...
            <h2 className="text-3xl sm:text-4xl font-bold mb-4 text-white">Veja o tipo de informação que <span style={{ color: "#9e3ffd" }}>orienta sua prospecção.</span></h2>
            <p className="text-base mb-10 max-w-2xl" style={{ color: "rgba(255,255,255,0.72)" }}>
                ...
                <tbody>
                  {[
                    ...
                    <tr key={i} style={{ borderTop: "1px solid rgba(255,255,255,0.05)", background: i % 2 === 0 ? "transparent" : "rgba(255,255,255,0.02)" }}>
                      ...
                      <td className="px-4 py-3" style={{ color: "rgba(255,255,255,0.55)" }}>{row[1]}</td>
                      <td className="px-4 py-3" style={{ color: "rgba(255,255,255,0.55)" }}>{row[2]}</td>
                      <td className="px-4 py-3" style={{ color: "#9e3ffd" }}>{row[3]}</td>
                      <td className="px-4 py-3">
                        <span className="px-2 py-0.5 rounded-full text-xs font-medium" style={{ background: row[4] === "Alta" ? "rgba(158,63,253,0.15)" : "rgba(200,143,245,0.15)", color: row[4] === "Alta" ? "#c88ff5" : "#a78bfa" }}>{row[4]}</span>
                      </td>
                    </tr>
                  ))}
                </tbody>
      ...
      <section className="px-5 py-20 border-t border-[rgba(255,255,255,0.04)]" style={{ background: "#09081e" }}>
        <div className="max-w-5xl mx-auto">
          <Reveal>
            ...
            <h2 className="text-3xl sm:text-4xl font-bold mb-4 text-white">Uma operação nacional com <span style={{ color: "#9e3ffd" }}>execução regional.</span></h2>
            <p className="text-base mb-10 max-w-2xl" style={{ color: "rgba(255,255,255,0.72)" }}>
            ...
            {REGIONS.map((r, i) => (
              <button key={r.key} onClick={() => setActiveRegion(i)}
                ...
                style={{
                  ...
                  color: activeRegion === i ? "#c88ff5" : "rgba(255,255,255,0.6)",
                }}>
                {r.emoji} {r.label}
              </button>
            ))}
          ...
          <Reveal>
            <div className="rounded-2xl p-6" style={{ background: cardBg, border: cardBorder }}>
              <div className="flex items-center gap-2 mb-5">
                ...
                <span className="font-semibold text-white">Segmentos prioritários em {REGIONS[activeRegion].label}</span>
              </div>
              <div className="flex flex-wrap gap-2">
                {REGIONS[activeRegion].items.map(item => (
                  <span key={item} className="px-3 py-1.5 rounded-lg text-xs" style={{ background: "rgba(158,63,253,0.08)", border: "1px solid rgba(158,63,253,0.2)", color: "rgba(255,255,255,0.72)" }}>
                    {item}
                  </span>
                ))}
              </div>
      ...
      {/* ── 10. PARTNER LEVELS ── */}
      <section className="relative px-5 py-20 overflow-hidden border-t border-[rgba(255,255,255,0.04)]" style={{ background: "#13112a" }}>
        <div className="max-w-5xl mx-auto">
          <Reveal>
            <SectionTag text="Níveis de parceiro" />
            <h2 className="text-3xl sm:text-4xl font-bold mb-3 text-white">Quanto melhor você performa, <span style={{ color: "#9e3ffd" }}>melhores ficam suas oportunidades.</span></h2>
            <p className="text-base mb-10 max-w-2xl" style={{ color: "rgba(255,255,255,0.72)" }}>
          ...
          <Reveal delay={100}>
            <div className="rounded-2xl p-6 grid grid-cols-2 sm:grid-cols-4 gap-5" style={{ background: `${LEVELS[activeLevel].color}08`, border: `1px solid ${LEVELS[activeLevel].color}25` }}>
              ...
              <div className="col-span-2 sm:col-span-4 pt-4" style={{ borderTop: "1px solid rgba(255,255,255,0.06)" }}>
                ...
                <p className="text-sm mt-1 text-white">{LEVELS[activeLevel].foco}</p>
              </div>
            </div>
          </Reveal>
        </div>
      ...
      <section className="px-5 py-20 border-t border-[rgba(255,255,255,0.04)]" style={{ background: "#09081e" }}>
        <div className="max-w-4xl mx-auto">
          <Reveal>
            ...
            <h2 className="text-3xl sm:text-4xl font-bold mb-4 text-white">Mais acesso vem de <span style={{ color: "#9e3ffd" }}>resultado, não de conversa.</span></h2>
            <p className="text-base mb-12 max-w-2xl" style={{ color: "rgba(255,255,255,0.72)" }}>
          ...
          <Reveal>
            <p className="text-xs font-bold uppercase tracking-widest mb-3" style={{ color: "#9e3ffd" }}>O parceiro executa</p>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-5">
              {[
                ...
                <div key={s.n} className="rounded-2xl p-5" style={{ background: cardBg, border: cardBorder }}>
                  ...
                  <p className="text-xs" style={{ color: "rgba(255,255,255,0.6)" }}>{s.desc}</p>
                </div>
              ))}
            </div>
          </Reveal>
          ...
          <Reveal delay={80}>
            <div className="flex justify-center mb-3">
              <div className="w-0.5 h-6" style={{ background: "rgba(158,63,253,0.4)" }} />
            </div>
          ...
          <Reveal delay={150}>
            <p className="text-xs font-bold uppercase tracking-widest mb-3" style={{ color: "rgba(255,255,255,0.4)" }}>O resultado</p>
              ...
              {[
                ...
                <div key={s.n} className="rounded-2xl p-5" style={{ background: cardBg, border: cardBorder }}>
                  <div className="w-9 h-9 rounded-full flex items-center justify-center font-bold text-sm text-white mb-3" style={{ background: s.color }}>
                    {(s as any).star ? <Star className="w-4 h-4" /> : s.n}
                  </div>
                  <p className="font-semibold text-white mb-1">{s.label}</p>
                  <p className="text-xs" style={{ color: "rgba(255,255,255,0.6)" }}>{s.desc}</p>
                </div>
          ...
          <Reveal delay={250}>
            <p className="text-xl font-bold mt-12 text-center text-white">
      ...
      <section className="px-5 py-20 border-t border-[rgba(255,255,255,0.04)]" style={{ background: "#13112a" }}>
        <div className="max-w-5xl mx-auto">
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(260px, 1fr))", gap: "2rem" }}>
            <Reveal>
              <div className="rounded-2xl p-7 h-full" style={{ background: "rgba(158,63,253,0.06)", border: "1px solid rgba(158,63,253,0.2)" }}>
                ...
                <p className="text-sm mb-5" style={{ color: "rgba(255,255,255,0.72)" }}>Você pode ser um bom perfil se já fala com empresários, síndicos, lojistas, administradores, gestores ou donos de negócios.</p>
            ...
            <Reveal delay={100}>
              <div className="rounded-2xl p-7 h-full" style={{ background: "rgba(255,80,80,0.05)", border: "1px solid rgba(255,80,80,0.2)" }}>
                ...
                <div className="mt-6 pt-5" style={{ borderTop: "1px solid rgba(255,255,255,0.06)" }}>
                  ...
                  <p className="text-sm" style={{ color: "rgba(255,255,255,0.72)" }}>Você coloca o processo em movimento.</p>
                </div>
              </div>
            </Reveal>
          </div>
      ...
      <section className="px-5 py-20 border-t border-[rgba(255,255,255,0.04)]" style={{ background: "#09081e" }}>
        <div className="max-w-3xl mx-auto text-center">
          <Reveal>
            ...
            <h2 className="text-3xl sm:text-4xl font-bold mb-4 text-white">Torne-se Parceiro <span style={{ color: "#9e3ffd" }}>Comercial Otima</span></h2>
          ...
          <Reveal delay={100}>
            <div className="text-left max-w-xl mx-auto" style={{ background: "rgba(255,255,255,0.07)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: "16px", padding: "32px" }}>
            ...
            <p className="text-sm text-center mt-5 max-w-md mx-auto" style={{ color: "rgba(255,255,255,0.55)" }}>
              Exemplo: um único cliente convertido com conta de R$15k/mês pode gerar <strong style={{ color: "#ffffff" }}>R$3.000/mês em comissão recorrente</strong>, mais de 10 vezes o valor da mensalidade.
            </p>
          </Reveal>
        </div>
      </section>
      ...
      <section className="px-5 py-20 border-t border-[rgba(255,255,255,0.04)]" style={{ background: "#13112a" }}>
        <div className="max-w-3xl mx-auto text-center">
          <Reveal>
            ...
            <h2 className="text-2xl sm:text-3xl font-bold mb-6 text-white">Você não está pagando por motivação. <span style={{ color: "#9e3ffd" }}>Está acessando um sistema comercial.</span></h2>
            <p className="text-base mb-6 max-w-xl mx-auto" style={{ color: "rgba(255,255,255,0.72)" }}>
            ...
            <div className="flex flex-col sm:flex-row gap-3 justify-center text-sm">
              {["Você não paga para recrutar pessoas.", "Você não paga por uma promessa de renda.", "Você acessa uma operação comercial real."].map(t => (
                <div key={t} className="rounded-xl px-4 py-3" style={{ background: cardBg, border: cardBorder, color: "rgba(255,255,255,0.72)" }}>{t}</div>
              ))}
            </div>
          </Reveal>
        </div>
      ...
      <section className="px-5 py-20 border-t border-[rgba(255,255,255,0.04)]" style={{ background: "#09081e" }}>
        <div className="max-w-3xl mx-auto">
          <Reveal>
            ...
            <h2 className="text-3xl sm:text-4xl font-bold mb-10 text-white">Dúvidas antes de <span style={{ color: "#9e3ffd" }}>candidatar-se?</span></h2>
          </Reveal>
              ...
              <Reveal key={i} delay={i * 50}>
                <div className="rounded-2xl overflow-hidden" style={{ border: cardBorder }}>
                  <button
                    ...
                    style={{ background: openFaq === i ? "rgba(158,63,253,0.08)" : "rgba(255,255,255,0.03)" }}
                  >
                  ...
                  {openFaq === i && (
                    <div className="px-6 pb-5 pt-4 text-sm leading-relaxed" style={{ color: "rgba(255,255,255,0.72)", borderTop: "1px solid rgba(255,255,255,0.05)" }}>
                      {item.a}
                    </div>
                  )}
                </div>
      ...
      {/* ── 16. FINAL CTA + FORM ── */}
      <section id="candidatura" className="relative px-5 py-20 overflow-hidden border-t border-[rgba(255,255,255,0.04)]" style={{ background: "#13112a" }}>
        <div className="max-w-2xl mx-auto">
          <Reveal>
            <div className="text-center mb-10">
              <SectionTag text="Candidatura" />
              <h2 className="text-3xl sm:text-4xl font-bold mb-4 text-white">Candidate-se ao <span style={{ color: "#9e3ffd" }}>Programa de Parceiros Otima</span></h2>
              <p className="text-sm max-w-lg mx-auto" style={{ color: "rgba(255,255,255,0.72)" }}>
          ...
          <Reveal delay={100}>
            <div className="rounded-2xl p-7 sm:p-9" style={{ background: "rgba(255,255,255,0.05)", border: "1px solid rgba(158,63,253,0.2)" }}>
              <PartnerForm />
            </div>
          </Reveal>
        </div>
      ...
      <div className="text-center py-8 px-5" style={{ borderTop: "1px solid rgba(255,255,255,0.04)" }}>
        <p style={{ fontSize: "0.9rem", color: "rgba(255,255,255,0.55)" }}>
          ...
          <a href="https://wa.me/5521997959777" target="_blank" rel="noopener noreferrer" style={{ color: "#9e3ffd", fontWeight: 600 }}>
      ...
      {/* Footer */}
      <footer className="px-5 py-8 text-center" style={{ borderTop: "2px solid #9e3ffd" }}>
        ...
        <p className="text-xs mt-3" style={{ color: "rgba(255,255,255,0.2)" }}>© 2026 Ótima Energia. Todos os direitos reservados.</p>
      </footer>
    </div>
  );
}
