...
function PartnerForm() {
  ...
  const inpStyle = { background: "white", border: "1px solid #e5e7eb", color: "#16163f" };
    ...
    <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
      {opts.map(o => (
        <button key={o.val} type="button" onClick={() => onChange(o.val)}
          ...
          style={{
            background: value === o.val ? "rgba(158,63,253,0.12)" : "rgba(22,22,63,0.03)",
            border: value === o.val ? "1px solid #9e3ffd" : "1px solid rgba(22,22,63,0.12)",
            color: value === o.val ? "#9e3ffd" : "#736d77"
          }}
        >
          {o.label}
        </button>
  ...
  if (submitted) {
    return (
      <div className="text-center py-12 px-4 space-y-4">
        ...
        <h3 className="text-2xl font-bold text-[#16163f]">Candidatura recebida.</h3>
        <p className="text-sm max-w-md mx-auto" style={{ color: "#736d77" }}>
  ...
  return (
    <div>
      ...
      <div className="flex gap-2 mb-8">
        {[1, 2, 3].map(n => (
          <div key={n} className="flex-1 h-1.5 rounded-full overflow-hidden" style={{ background: "rgba(22,22,63,0.1)" }}>
      ...
      {step === 1 && (
        <div className="space-y-4">
          <h3 className="text-lg font-bold text-[#16163f]">Dados básicos</h3>
          {[
            ...
            <div key={field.key}>
              <label className="block text-xs font-medium mb-1.5" style={{ color: "#736d77" }}>{field.label}</label>
          ...
          <div>
            <label className="block text-xs font-medium mb-1.5" style={{ color: "#736d77" }}>Estado</label>
      ...
      {step === 2 && (
        <div className="space-y-5">
          <h3 className="text-lg font-bold text-[#16163f]">Perfil comercial</h3>
          <div>
            <label className="block text-xs font-medium mb-2" style={{ color: "#736d77" }}>Você já vendeu B2B?</label>
            {radioOpts("b2b", [{ val: "sim", label: "Sim" }, { val: "nao", label: "Não" }], f2.vendeuB2b, v => setF2({ ...f2, vendeuB2b: v }))}
          </div>
          <div>
            <label className="block text-xs font-medium mb-2" style={{ color: "#736d77" }}>Qual sua experiência comercial?</label>
            {radioOpts("exp", [
          ...
          <div>
            <label className="block text-xs font-medium mb-2" style={{ color: "#736d77" }}>Horas por semana disponíveis</label>
            {radioOpts("horas", [{ val: "5-10", label: "5–10h" }, { val: "10-20", label: "10–20h" }, { val: "20+", label: "20h+" }], f2.horas, v => setF2({ ...f2, horas: v }))}
          </div>
          <div>
            <label className="block text-xs font-medium mb-2" style={{ color: "#736d77" }}>Você tem rede local com empresários?</label>
          ...
          <div className="flex gap-3">
            <button onClick={() => setStep(1)} className="flex-1 py-3 rounded-xl text-sm font-medium" style={{ background: "rgba(22,22,63,0.04)", border: "1px solid #e5e7eb", color: "#736d77" }}>Voltar</button>
      ...
      {step === 3 && (
        <div className="space-y-5">
          <h3 className="text-lg font-bold text-[#16163f]">Confirmação e compromisso</h3>
          {[
            ...
            <button key={item.key} type="button" onClick={() => setF3({ ...f3, [item.key]: !(f3 as any)[item.key] })}
              ...
              style={{ background: (f3 as any)[item.key] ? "rgba(158,63,253,0.12)" : "rgba(22,22,63,0.03)", border: `1px solid ${(f3 as any)[item.key] ? "rgba(158,63,253,0.4)" : "rgba(22,22,63,0.1)"}` }}>
              <div className="w-5 h-5 rounded-full flex items-center justify-center shrink-0 mt-0.5" style={{ background: (f3 as any)[item.key] ? "#9e3ffd" : "rgba(22,22,63,0.05)", border: `1px solid ${(f3 as any)[item.key] ? "#9e3ffd" : "rgba(22,22,63,0.15)"}` }}>
                {(f3 as any)[item.key] && <CheckCircle2 className="w-3.5 h-3.5 text-white" />}
              </div>
              <span className="text-sm" style={{ color: (f3 as any)[item.key] ? "#16163f" : "#736d77" }}>{item.label}</span>
            </button>
          ))}
          <div>
            <label className="block text-xs font-medium mb-1.5" style={{ color: "#736d77" }}>Por que você quer ser Parceiro Otima?</label>
          ...
          <div className="flex gap-3">
            <button onClick={() => setStep(2)} className="flex-1 py-3 rounded-xl text-sm font-medium" style={{ background: "rgba(22,22,63,0.04)", border: "1px solid #e5e7eb", color: "#736d77" }}>Voltar</button>
...
export default function Parceiros() {
  ...
  return (
    <div style={{ background: "#fff", color: "#16163f", fontFamily: "Inter, system-ui, sans-serif", overflowX: "hidden" }}>
      ...
      <header className="sticky top-0 z-50 px-5 py-3 flex items-center justify-between" style={{ background: "rgba(255,255,255,0.97)", backdropFilter: "blur(16px)", borderBottom: "1px solid #e5e7eb" }}>
        <Link href="/">
          <div className="flex items-center gap-2 cursor-pointer">
            ...
            <span className="font-bold text-[#16163f] text-sm">Ótima Energia</span>
          </div>
        </Link>
      ...
      {/* ── 2. PROBLEM ── */}
      <section className="px-5 py-20" style={{ background: "#eee7f1" }}>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr style={{ background: "#f5f0fa" }}>
                  <th className="text-left px-5 py-3.5 font-semibold" style={{ color: "#a0aec0", width: "50%" }}>Antes da Otima</th>
            ...
            {[
              ...
              <Reveal key={i} delay={i * 60}>
                <div className="rounded-2xl p-5 h-full transition-all duration-200 hover:scale-[1.02]" style={{ background: cardBg, border: cardBorder }}>
                  ...
                  <h3 className="font-semibold mb-1.5 text-[#16163f]">{card.title}</h3>
                  <p className="text-sm leading-relaxed" style={{ color: "#736d77" }}>{card.desc}</p>
                </div>
              </Reveal>
            ))}
          </div>
      ...
      {/* ── 4. HOW IT WORKS ── */}
      <section id="como-funciona" className="px-5 py-20" style={{ background: "#fff" }}>
          ...
          {[
            ...
            <Reveal key={step.num} delay={i * 50}>
              <div className="flex gap-5 pb-8 relative">
                ...
                <div className="pb-2">
                  <h3 className="font-semibold text-[#16163f] mb-1">{step.title}</h3>
                  <p className="text-sm leading-relaxed" style={{ color: "#736d77" }}>{step.desc}</p>
                </div>
              </div>
            </Reveal>
          ))}
      ...
      <div className="max-w-5xl mx-auto">
        <Reveal>
          ...
          <p className="text-base mb-4 max-w-2xl" style={{ color: "#736d77" }}>
          ...
          <p className="text-base mb-10 max-w-2xl" style={{ color: "#736d77" }}>
      ...
      <section className="px-5 py-20" style={{ background: "rgba(200,143,245,0.07)", borderTop: "1px solid rgba(158,63,253,0.12)", borderBottom: "1px solid rgba(200,143,245,0.12)" }}>
        <div className="max-w-4xl mx-auto text-center">
          <Reveal>
            ...
            <p className="text-base mb-6 max-w-2xl mx-auto" style={{ color: "#736d77" }}>
          ...
          <Reveal delay={100}>
            <div className="flex flex-col sm:flex-row items-center justify-center gap-6 my-10">
              <div className="rounded-2xl px-8 py-6" style={{ background: "rgba(255,80,80,0.08)", border: "1px solid rgba(255,80,80,0.2)" }}>
                ...
                <p className="text-sm mt-1" style={{ color: "#736d77" }}>é contacto.</p>
              </div>
              <ArrowRight className="w-6 h-6 rotate-90 sm:rotate-0" style={{ color: "#c4c4c4" }} />
              <div className="rounded-2xl px-8 py-6" style={{ background: "rgba(158,63,253,0.1)", border: "1px solid rgba(158,63,253,0.3)" }}>
                ...
                <p className="text-sm mt-1" style={{ color: "#736d77" }}>vira oportunidade.</p>
              </div>
            </div>
          </Reveal>
        </div>
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <Reveal delay={100}>
              ...
              <thead>
                <tr style={{ background: "#f5f0fa" }}>
                  {["Empresa", "Cidade", "Segmento", "Conta estimada", "Prioridade"].map(h => (
                    <th key={h} className="text-left px-4 py-3 font-semibold text-xs" style={{ color: "#a0aec0" }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {[
                  ...
                  <tr key={i} style={{ borderTop: "1px solid rgba(22,22,63,0.08)", background: i % 2 === 0 ? "transparent" : "rgba(22,22,63,0.03)" }}>
                    <td className="px-4 py-3 font-medium text-[#16163f]">{row[0]}</td>
                    <td className="px-4 py-3" style={{ color: "#736d77" }}>{row[1]}</td>
                    <td className="px-4 py-3" style={{ color: "#736d77" }}>{row[2]}</td>
          ...
          <p className="text-xs mt-3 text-center" style={{ color: "#a0aec0" }}>
            ...
            {REGIONS.map((r, i) => (
              <button key={r.key} onClick={() => setActiveRegion(i)}
                ...
                style={{
                  ...
                  color: activeRegion === i ? "#9e3ffd" : "#736d77",
                }}>
                {r.emoji} {r.label}
              </button>
            ))}
          ...
          <Reveal>
            <div className="rounded-2xl p-6" style={{ background: cardBg, border: cardBorder }}>
              <div className="flex items-center gap-2 mb-5">
                ...
                <span className="font-semibold text-[#16163f]">Segmentos prioritários em {REGIONS[activeRegion].label}</span>
              </div>
              <div className="flex flex-wrap gap-2">
                {REGIONS[activeRegion].items.map(item => (
                  <span key={item} className="px-3 py-1.5 rounded-lg text-xs" style={{ background: "rgba(158,63,253,0.08)", border: "1px solid rgba(158,63,253,0.2)", color: "#16163f" }}>
                    {item}
                  </span>
                ))}
              </div>
      ...
      {/* ── 10. PARTNER LEVELS ── */}
      <section className="relative px-5 py-20 overflow-hidden" style={{ background: "#fff" }}>
        <div className="absolute inset-0 pointer-events-none">
          ...
          <div className="absolute inset-0" style={{ background: "linear-gradient(to bottom, #fff 0%, transparent 12%, transparent 88%, #fff 100%)" }} />
        </div>
      <div className="max-w-5xl mx-auto">
        <Reveal>
          ...
          {LEVELS.map((lvl, i) => (
            <Reveal key={lvl.num} delay={i * 60}>
              <button onClick={() => setActiveLevel(i)} className="w-full rounded-2xl p-5 text-left transition-all duration-200 hover:scale-[1.02]"
                ...
                <p className="font-semibold text-[#16163f] text-sm mb-0.5">{lvl.name}</p>
                <p className="text-xs" style={{ color: "#a0aec0" }}>{lvl.subtitle}</p>
              </button>
            </Reveal>
          ))}
        </div>
        ...
        <Reveal delay={100}>
          <div className="rounded-2xl p-6 grid grid-cols-2 sm:grid-cols-4 gap-5" style={{ background: `${LEVELS[activeLevel].color}08`, border: `1px solid ${LEVELS[activeLevel].color}25` }}>
            {[
              ...
              <div key={item.label}>
                <p className="text-xs mb-1" style={{ color: "#a0aec0" }}>{item.label}</p>
                <p className="text-lg font-bold" style={{ color: LEVELS[activeLevel].color }}>{item.value}</p>
              </div>
            ))}
            <div className="col-span-2 sm:col-span-4 pt-4" style={{ borderTop: "1px solid rgba(22,22,63,0.08)" }}>
              <p className="text-xs" style={{ color: "#a0aec0" }}>Foco comercial</p>
              <p className="text-sm mt-1" style={{ color: "#16163f" }}>{LEVELS[activeLevel].foco}</p>
            </div>
          </div>
        </Reveal>
      </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          <Reveal>
            <div className="rounded-2xl p-7 h-full" style={{ background: "rgba(158,63,253,0.05)", border: "1px solid rgba(158,63,253,0.15)" }}>
              ...
              <h3 className="text-2xl font-bold mb-4 text-[#16163f]">Este programa é para quem <span style={{ color: "#9e3ffd" }}>sabe executar.</span></h3>
              <p className="text-sm mb-5" style={{ color: "#736d77" }}>Você pode ser um bom perfil se já fala com empresários, síndicos, lojistas, administradores, gestores ou donos de negócios.</p>
              <ul className="space-y-2">
                {["Consultores comerciais", "Vendedores autônomos", "Representantes comerciais", "Corretores", "Empreendedores locais", "Ex-gerentes comerciais", "Profissionais com rede empresarial", "Quem já vende B2B ou quer vender com processo"].map(item => (
                  <li key={item} className="flex items-center gap-2 text-sm" style={{ color: "#16163f" }}>
                    <CheckCircle2 className="w-3.5 h-3.5 shrink-0" style={{ color: "#9e3ffd" }} /> {item}
                  </li>
                ))}
              </ul>
              <p className="text-xs mt-5" style={{ color: "#a0aec0" }}>Você não precisa ser especialista em energia. Mas precisa saber abrir conversa, pedir a conta e seguir um processo comercial.</p>
            </div>
          </Reveal>
          ...
          <Reveal delay={100}>
            <div className="rounded-2xl p-7 h-full" style={{ background: "rgba(255,80,80,0.04)", border: "1px solid rgba(255,80,80,0.15)" }}>
              ...
              <h3 className="text-2xl font-bold mb-4 text-[#16163f]">Este programa não é para <span style={{ color: "rgba(255,120,120,0.9)" }}>todo mundo.</span></h3>
              <ul className="space-y-2">
                {[
                  ...
                  <li key={item} className="flex items-center gap-2 text-sm" style={{ color: "#736d77" }}>
                    <XCircle className="w-3.5 h-3.5 shrink-0" style={{ color: "rgba(255,100,100,0.7)" }} /> {item}
                  </li>
                ))}
              </ul>
              <div className="mt-6 pt-5" style={{ borderTop: "1px solid rgba(22,22,63,0.08)" }}>
                <p className="text-sm font-medium text-[#16163f]">A Otima entrega a estrutura.</p>
                <p className="text-sm" style={{ color: "#736d77" }}>Você coloca o processo em movimento.</p>
              </div>
            </div>
          </Reveal>
        </div>
      ...
      <div className="max-w-3xl mx-auto text-center">
        <Reveal>
          ...
          <p className="text-base mb-6 max-w-xl mx-auto" style={{ color: "#736d77" }}>
          ...
          <div className="flex flex-col sm:flex-row gap-3 justify-center text-sm">
            {["Você não paga para recrutar pessoas.", "Você não paga por uma promessa de renda.", "Você acessa uma operação comercial real."].map(t => (
              <div key={t} className="rounded-xl px-4 py-3" style={{ background: cardBg, border: cardBorder, color: "#736d77" }}>{t}</div>
            ))}
          </div>
        </Reveal>
      </div>
      ...
      <section id="candidatura" className="relative px-5 py-20 overflow-hidden" style={{ borderTop: "1px solid rgba(158,63,253,0.12)" }}>
        ...
        <div className="max-w-2xl mx-auto">
          ...
          <Reveal delay={100}>
            <div className="rounded-2xl p-7 sm:p-9" style={{ background: "#f5f0fa", border: "1px solid rgba(158,63,253,0.15)" }}>
              <PartnerForm />
            </div>
          </Reveal>
        </div>
      ...
      <footer className="px-5 py-8 text-center" style={{ borderTop: "1px solid rgba(22,22,63,0.08)" }}>
        ...
        <div className="flex items-center justify-center gap-4 text-xs" style={{ color: "#a0aec0" }}>
          <Link href="/privacy"><span className="hover:text-[#9e3ffd] transition-colors cursor-pointer">Privacidade</span></Link>
