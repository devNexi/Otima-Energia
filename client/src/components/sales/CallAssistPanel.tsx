...
function CopyBtn({ text }: CopyBtnProps) {
  ...
  return (
    <button onClick={doCopy} className="flex items-center gap-1 px-2 py-0.5 rounded text-[11px] transition-all"
      style={{ background: copied ? "rgba(34,197,94,0.15)" : "rgba(255,255,255,0.06)", color: copied ? "#22c55e" : "rgba(255,255,255,0.45)", border: "1px solid rgba(255,255,255,0.1)" }}>
...
export function CallAssistPanel({ lead, compact = false, onScriptViewed, scriptViewed = false }: Props) {
  ...
  const items: AssistItem[] = [
    { icon: <Target size={14} />, label: "Objetivo da Chamada", content: lead.call_objective },
    { icon: <Zap size={14} />, label: "Por Que Agora", content: lead.call_why_now_angle },
    { icon: <User size={14} />, label: "Alvo", content: lead.dm_name ? `${lead.dm_name} · ${lead.dm_role} · ${lead.dm_direct_phone_best || "Buscar número"}` : "Decisor não identificado — identificar na triagem" },
    { icon: <MessageSquare size={14} />, label: "Diga Isso Primeiro", content: lead.suggested_opening_angle, copyable: true },
    { icon: <AlertTriangle size={14} />, label: "Aviso de Confiança", content: lead.call_trust_warning, variant: "amber" },
    { icon: <HelpCircle size={14} />, label: "Primeira Pergunta", content: lead.suggested_first_question, copyable: true },
    { icon: <FileText size={14} />, label: "Ângulo do Pedido de Conta", content: lead.suggested_bill_ask_angle, copyable: true },
    { icon: <RotateCcw size={14} />, label: "Script para Gatekeeper", content: lead.gatekeeper_script, copyable: true },
  ];
      ...
      {!compact && (
        <div>
          ...
          >
            ...
            <span className="text-[11px] font-semibold uppercase tracking-wide" style={{ color: "rgba(255,255,255,0.4)" }}>Resumo da Última Interação</span>
          ...
          <AnimatePresence>
            {openObjection && (
              ...
              >
                ...
                <CopyBtn text={objectionResponses[openObjection]} />
              </motion.div>
            )}
          </AnimatePresence>
        </div>
