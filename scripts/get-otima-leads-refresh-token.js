#!/usr/bin/env node
/**
 * One-time setup script — generates a Google OAuth2 refresh token for lead capture.
 * Run with:  node scripts/get-otima-leads-refresh-token.js
 *
 * Scopes:
 *   - https://www.googleapis.com/auth/spreadsheets
 *   - https://www.googleapis.com/auth/drive.file
 *
 * Requires OTIMA_LEADS_GOOGLE_CLIENT_ID and OTIMA_LEADS_GOOGLE_CLIENT_SECRET
...
if (!clientId || !clientSecret) {
  console.error("\nERROR: OTIMA_LEADS_GOOGLE_CLIENT_ID and OTIMA_LEADS_GOOGLE_CLIENT_SECRET must be set as environment variables.");
...
console.log("\n─────────────────────────────────────────────────────────────────");
console.log("  Ótima Energia — Lead Capture OAuth2 Refresh Token Generator");
console.log("─────────────────────────────────────────────────────────────────");
console.log("\nThis token is ONLY for lead capture (Google Sheets + Drive).");
console.log("For Google Ads, use scripts/get-otima-ads-refresh-token.js instead.\n");
...
rl.question("Step 3 — Paste the full redirect URL (or just the code= value) here:\n> ", async (input) => {
  ...
  try {
    ...
    console.log("  Secret name : OTIMA_LEADS_GOOGLE_REFRESH_TOKEN");
