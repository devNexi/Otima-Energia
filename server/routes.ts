...
<body>
  ...
  app.get("/api/deals/:dealId/blind-auction", async (req, res) => {
    ...
    try {
      ...
      // Get unique suppliers from active playbooks — deduplicate by supplierId, exclude DEMO
      const allPlaybooks = await storage.getAllActivePlaybooks();
      const uniqueSupplierIds = [...new Set(allPlaybooks.map(p => p.supplierId))];
      const allFetched = await Promise.all(uniqueSupplierIds.map(id => storage.getSupplier(id)));
  ...
  app.post("/api/admin/seed-suppliers", async (req, res) => {
    if (!await validateDealOsSession(req, res)) return;
    const sessionId = req.headers["x-session-id"] as string;
    const session = await storage.getAdminSession(sessionId);
    const user = session ? await storage.getUser(session.userId) : null;
    ...
    try {
      ...
      for (const c of CONTACTS) {
        ...
        if (existing.length === 0) {
          await db.insert(supplierContacts).values({ supplierId, name: c.name, role: c.role, department: c.department, email: c.email, isPrimary: c.isPrimary, preferredFormat: 'email', isActive: true });
          results.contacts++;
        }
      }

      console.log('[SeedSuppliers] Results:', results);
      await logAuditEvent({ actor: user.username, actorRole: user.role, actorIp: req.ip || null, userAgent: req.get("User-Agent") || null, action: "SUPPLIERS_SEEDED", entityType: "suppliers", entityId: null, detailsJson: results });
  ...
  app.get("/api/download/portal-parser", async (req: Request, res: Response) => {
    ...
    const filePath = resolve("portal-parser.zip");
