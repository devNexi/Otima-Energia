...
- **Bill-First Architecture**: Enforces a structured deal journey: Bill → ECOS → Dossier → RFQ. Bills are the foundational input for all subsequent processes. Treats all bill-adjacent documents (consumption history, NF3e, demonstrativos) as RFQ data sources.
  ...
  - **Debug Panel**: Ops/admin-only expandable panel showing parser URL, latency, doc kind, pages, text source, fieldReasons, extraction summary (found vs missing fields), and raw JSON download. Confidence displayed as 0-1 decimal with normalizeConfidence() handling legacy double-multiplied values.
