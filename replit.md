...
- **Deal Tracks**: Two canonical product lines: **GD** (Geração Distribuída, stored as `GDL` in DB) and **ACL** (Livre). ACR/OTHER track types deprecated — hidden from creation UI but existing tracks remain readable. Display mapping via `TRACK_TYPE_DISPLAY` constant; creation gated by `CREATABLE_TRACK_TYPES`. Each track has a unified comparison panel (`TrackComparisonPanel`) showing ranked supplier offers with "Best Fit" recommendation.
