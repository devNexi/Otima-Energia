...
- **ECOS™ System**: AI for contract analysis, lead snapshots, renewal tracking, and client energy profiles. PRC resolver prioritizes current-year prices (5-stage: exact month → current year submarket → current year global → older year submarket → older year global). Stale pricing (older year) triggers warning banner, forces LOW confidence, and logs console warning. ECOS UI shows PRC reference month, term breakdown table with product/year/price/ref columns, and year warning badge.
  - **DealReadinessCard ECOS Action**: "Gerar ECOS" and "Resolver" buttons in the Bill-First Journey strip (`DealReadinessCard.tsx`) call `POST /api/deals/:dealId/ecos/generate` directly via `useMutation`. Shows loading spinner during generation, toast on success (with query invalidation) or failure (with blocker details). Previously these buttons only navigated to the assembly tab.
...
- **RFQ Email Dispatch**: `POST /api/deals/:dealId/rfq-dispatches` now immediately sends email via nodemailer (Zoho SMTP, `SMTP_PASS` env var) for EMAIL channel dispatches. Returns `emailResult: {status: 'SENT'|'FAILED'|'NO_EMAIL'|'NO_SMTP'}` + `supplierEmail`. If SENT, dispatch is auto-marked SENT with SLA timer. `POST /api/rfq-dispatches/:id/resend` retries email for any dispatch. Dossier gate updated to field-based check (legalName+cnpj+distributor+annualConsumption+connectionType) instead of status check. UI shows per-dispatch email delivery badge (Enviado/Falhou/Pendente), copy button on all rows, "Reenviar" button for DRAFT EMAIL dispatches, and SMTP warning banner with copyable message when email is not configured.
  - **Reply Tracking**: Every outgoing RFQ email subject is appended with `[RFQ-{dispatchId}]` (e.g. `Solicitação de Cotação — XPTO [RFQ-47]`). `Reply-To` header set to both `renan@otimaenergia.com` and `notificacoes@otimaenergia.com`. Each SENT dispatch row shows a blue inbox notice indicating where replies arrive and the RFQ reference to look for. "Registrar Cotação" button styled as a prominent green CTA.
  - **Inbound Webhook**: `POST /api/intake/rfq-reply` — receives Zoho Mail webhook payloads, extracts `[RFQ-{id}]` from subject, auto-marks the dispatch as RESPONDED. Protected by `x-zoho-intake-key` header (same key as Zoho CRM intake). Idempotent (ignores already-RESPONDED dispatches).
